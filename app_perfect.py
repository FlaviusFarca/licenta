...
def generate_pdf_report(r, original_pdf_bytes=None):
    ...
    class PDF(FPDF):
        def header(self):
            ...
            self.cell(0, 10, "TEXTSCAN AI - RAPORT DE ANALIZA", align="C", new_x="LMARGIN", new_y="NEXT")
            self.ln(5)
    ...
    pdf.cell(0, 8, f"Rezultat global: {r['label']} (AI: {r['prob_ai']:.2f}% | Uman: {r['prob_human']:.2f}%)", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 8, f"Model folosit: {r['model']}", new_x="LMARGIN", new_y="NEXT")
    pdf.cell(0, 8, f"Fragmente analizate: {r.get('num_chunks', 1)}", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(10)
    
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 10, "TEXT ANALIZAT (fragmentele AI sunt evidentiate cu rosu):", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(2)
...
if st.session_state.last_result is not None:
    ...
    with exp_c2:
        ...
        base_name = f"TextScan_{orig_name}" if orig_name else f"textscan_{r['model_key']}_{ts}"
...
if st.session_state.get("batch_results"):
    ...
    for br in st.session_state.batch_results:
        ...
        with st.expander(f"{br['source_filename']}  —  {br['label']} ({br['prob_ai']:.1f}% AI)", expanded=False):
            render_result(br["prob_ai"], br["prob_human"], br["model"], br.get("num_chunks", 1))
