...
# Hugging Face Repositories (Cloud Fallback)
HF_MODEL_V1 = "FlaviusA/roberta-v1-ai-detector"
HF_MODEL_V2 = "FlaviusA/roberta-v2-ai-detector"

_v1_same   = os.path.join(BASE_DIR, "roberta2")
_v1_parent = os.path.join(BASE_DIR, "..", "roberta2")
if os.path.exists(os.path.join(_v1_same, "config.json")):
    MODEL_V1_DIR = _v1_same
elif os.path.exists(os.path.join(_v1_parent, "config.json")):
    MODEL_V1_DIR = _v1_parent
else:
    MODEL_V1_DIR = HF_MODEL_V1

_v2_same   = os.path.join(BASE_DIR, "roberta3")
_v2_parent = os.path.join(BASE_DIR, "..", "roberta3")
if os.path.exists(os.path.join(_v2_same, "config.json")):
    MODEL_V2_DIR = _v2_same
elif os.path.exists(os.path.join(_v2_parent, "config.json")):
    MODEL_V2_DIR = _v2_parent
else:
    MODEL_V2_DIR = HF_MODEL_V2
...
/* ── Tab-uri principale (Meniu) ───────────────────────────── */
button[data-baseweb="tab"] p, button[data-baseweb="tab"] {
    font-size: 1.15rem !important; /* Mărire cu ~3-4px față de standard */
    font-weight: 600 !important;
}
</style>
""",
...
with tab_file:
                ...
                if uf.name.endswith(".txt"):
                    ...
                    if not df.empty:
                        ...
                        sel_col = st.selectbox(
                            ...
                            help="Select the column that contains the text you want to analyze. The system will extract and evaluate the text row by row from this specific column."
                        )
    ...
    if loaded_files:
        ...
        if batch_btn and selected_files_to_run:
            ...
            st.toast(f"Analysis complete for {len(selected_files_to_run)} files!")
...
with tab_guide:
    ...
    with col_g1:
        st.info("**RoBERTa v1 (ChatGPT only)**", icon=None)
        st.markdown("""
**Best for:** Texts suspected to be generated by ChatGPT (versions 3.5 or 4).

**Strengths:**
- Achieves near-perfect accuracy (over 99%) for ChatGPT texts.
- Highly specialized on OpenAI's specific writing style.

**Limitations:**
- May struggle to detect text from other AI generators (like Claude or Gemini).
        """)

    with col_g2:
        st.success("**RoBERTa v2 (Multi-model)** - Default Choice", icon=None)
        st.markdown("""
**Best for:** General internet text or when the AI source is unknown.

**Strengths:**
- Trained on a highly diverse dataset (Llama, Claude, Gemini, Mistral, ChatGPT).
- Offers superior cross-model generalization.
- Highly robust across the entire AI landscape.

**Limitations:**
- Slightly lower confidence peak (approx 96%) compared to the highly-specialized v1.
        """)

    st.markdown("---")
    
    with st.expander("Best Practices: How to avoid false positives", expanded=False):
        st.markdown("""
- **Analyze sufficient length:** The models perform best on texts longer than 80 words. Very short sentences lack sufficient linguistic patterns for accurate detection.
- **Avoid pure lists or code:** Formatted data, bulleted lists, and programming code can confuse the linguistic heuristics.
- **Understand the probabilities:** The tool provides a statistical probability based on training data, not an absolute certainty. Always review the highlighted segments contextually.
        """)
...
with tab_text:
        ...
        if should_detect and active_text and active_text.strip():
            if selected_key == "compare":
                ...
                with st.spinner("Running both models..."):
                    ...
                    st.toast("Compare analysis complete!")
            else:
                st.session_state.last_compare = None
    ...
    <div class="empty-state">
        <svg width="56" height="56" viewBox="0 0 24 24" fill="none" stroke="#94a3b8" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" style="margin-bottom:20px;">
            <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path>
            <polyline points="14 2 14 8 20 8"></polyline>
            <circle cx="10" cy="13" r="2"></circle>
            <line x1="11.5" y1="14.5" x2="15" y2="18"></line>
        </svg>
        <p class="empty-state-title">System Ready for Analysis</p>
        <p class="empty-state-sub">Please paste your text into the editor, write manually, or <strong>upload a document in the dedicated tab</strong> to begin linguistic analysis.</p>
    </div>
    """,
                    unsafe_allow_html=True,
                )
